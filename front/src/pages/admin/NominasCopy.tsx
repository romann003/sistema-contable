...
import { Bonificacion, Nomina, Status, emptyBonificacion, emptyNomina, typeStatus } from '../../layout/elements/InitialData';
...
export default function NominaPage() {
    ...
    const { periodos, getPeriodos, setPeriodos,
        bonificaciones, getBonificaciones, createBonificacion, deleteBonificacion, updateBonificacion
    } = useNominaDatos();
    const { employees, getEmployees, setEmployees } = useEmployees();
    const { nominas, getNominas, createNomina, deleteNomina, updateNomina, nominaId, setNominaId, getNomina } = useNominas();
    ...
    const descriptionBodyTemplate = (rowData: Bonificacion) => {
        return <TemplatesBody.ColumnTextBody value={rowData.descripcion} />
    };

    const cantidadBodyTemplate = (rowData: Bonificacion) => {
        return <TemplatesBody.ColumnTextBody value={rowData.cantidad} />
    };

    const datosCreatedAtBodyTemplate = (rowData: Bonificacion) => {
        return <TemplatesBody.ColumnDateBody value={rowData.createdAt} />
    };

    const datosUpdatedAtBodyTemplate = (rowData: Bonificacion) => {
        return <TemplatesBody.ColumnDateBody value={rowData.updatedAt} />
    };
    ...
    const ToolbarBonificacionTemplate = () => {
        return (
            <div className="flex flex-wrap gap-2">
                <Button label="Agregar Bonificación" size='small' icon="pi pi-plus" severity="success" onClick={openBoni} />
            </div>
        );
    };
    ...
    return (
        <div>
            ...
            <Dialog visible={datosNominaDialog} style={{ width: '70rem', height: '42rem', minWidth: '50rem', maxWidth: '90vw', minHeight: '40rem', maxHeight: '90vh' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header={`Detalles de la Nomina: ${nominaId}`} modal className="p-fluid" onHide={hideDatosNomina} dismissableMask={false} blockScroll={true} closeOnEscape={false}
            >
                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    <TabView style={{ flex: 1, overflow: 'auto' }}>

                        <TabPanel className='w-full' header="Header I" headerTemplate={tab1HeaderTemplate}>
                            <Toolbar className="my-4" left={ToolbarBonificacionTemplate}></Toolbar>
                            ...
                            <DataTable
                                ...
                                emptyMessage="No se encontraron bonificaciones."
                                paginator rows={15}
                                paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                                currentPageReportTemplate={`Mostrando {first} - {last} de {totalRecords} bonificaciones`}
                            >
                                <Column header="ID" body={(rowData) => <span>{bonificaciones.indexOf(rowData) + 1}</span>} />
                            ...
                            {/* <DataTableCrud
                                ...
                                columns={[
                                    ...
                                    // { field: 'createdAt', header: 'Creado el', body: datosCreatedAtBodyTemplate, dataType: 'date', filter: false },
                                    // { field: 'updatedAt', header: 'Ultima Actualización', body: datosUpdatedAtBodyTemplate, filter: false, dataType: 'date' },
                                ]}
                                size='12rem'
                                actionBodyTemplate={actionBodyTemplate}
                            /> */}
